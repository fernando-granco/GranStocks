...
- **Multi-Asset Tracking**: Track both US Stocks (via Finnhub) and Cryptocurrencies (via Binance) in a unified dashboard.
...
- **Group AI Analysis**: Use your own AI keys (OpenAI, Claude, Gemini, etc.) to generate macro narratives for an entire group of stocks at once.
...
### 1. Data Aggregation
The platform interfaces with **Finnhub.io** for equities and **Binance** for digital assets. It pulls deep historical daily candles to ensure technical indicators are statistically significant.
...
### 3. Bring-Your-Own-Key (BYOK) AI
Instead of paying for expensive monthly AI subscriptions, users plug in their own API keys from providers like OpenAI (GPT-4o), Anthropic (Claude 3.5), or DeepSeek. The "Narrative Engine" sends the *technical indicators* (not just prices) to the AI to get a reasoned analysis of the asset's current state.
